...
\begin{itemize}
    \item Expanded Amazon Connect to support Tokyo and Osaka, providing automated traffic redirection during outages.
...
\begin{itemize}
    ...
    \item Launched the Skill Builder public profile page, displaying user progress data from game-based learning modules, with UI built in Tailwind CSS and TypeScript. Used Cognito, CloudFront, API Gateway, Lambda, SQS, DynamoDB.
    \item Enabled users to share achievements on LinkedIn with a badge image preview through server-generated meta tags.
\end{itemize}

\textbf{Orkes}
    \hfill Cupertino, CA, USA
    \\ Software Engineer (remote contractor)
    \hfill \textit{01/2022 - 07/2023}
\begin{itemize}
    ...
    \item Improved throughput by 20\%, with a long-running thread pool per worker, to periodically poll for workflow tasks in batches, then start a thread per task, making parallel requests (HTTP, gRPC).
    \item Contributed to Netflix OSS by adding missing message acknowledgment requests after batch requests on the server.
\end{itemize}
...
\begin{itemize}
    \item Resized the EC2 fleet for invoice services ahead of Black Friday using expected peak usage and maximum requests per host. Based on historical load tests integrated into the deployment pipeline.
    \item Extended Java microservices and React UI to show state-specific invoice expiration dates for FBA sellers in Brazil.
...
\begin{itemize}
    \item Built a matching engine to reproduce stock exchange behavior with FIX and validate strategies through backtests.
